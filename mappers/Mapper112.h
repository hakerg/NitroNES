...
public:
	...
	bool cpuMapRead(uint16_t a, uint32_t& mapped, uint8_t&) override {
		...
		uint8_t b;
		if      (a < 0xA000) b = R[0];
		else if (a < 0xC000) b = R[1];
		else if (a < 0xE000) b = (uint8_t)(total8k - 2);
	...
	void cpuMapWrite(uint16_t a, uint32_t&, uint8_t data) override {
		...
		if      (reg == 0x8000) sel = data & 0x07;
		else if (reg == 0xA000) { if (sel < 8) R[sel] = data; }
		else if (reg == 0xE000) mirrorMode = (data & 0x01) ? Mirroring::HORIZONTAL : Mirroring::VERTICAL;

	}
	bool ppuMapRead(uint16_t a, uint32_t& mapped) override {
		if (a > 0x1FFF) return false;
		uint8_t b;
		if      (a < 0x0800) b = R[2] & 0xFE;
		else if (a < 0x1000) b = R[3] & 0xFE;
		else if (a < 0x1400) { mapped = (uint32_t)R[4] * 0x0400 + (a & 0x03FF); return true; }
		else if (a < 0x1800) { mapped = (uint32_t)R[5] * 0x0400 + (a & 0x03FF); return true; }
		else if (a < 0x1C00) { mapped = (uint32_t)R[6] * 0x0400 + (a & 0x03FF); return true; }
		else                 { mapped = (uint32_t)R[7] * 0x0400 + (a & 0x03FF); return true; }
