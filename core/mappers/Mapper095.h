...
public:
	...
	Mirroring mirror() const override {
		...
		if (a == b && c == d) return Mirroring::HORIZONTAL;
		if (a == b && b == c && c == d) return a ? Mirroring::ONESCREEN_HI : Mirroring::ONESCREEN_LO;
...
private:
	void updateBanks() {
		...
		auto r = [&](int i) -> uint32_t { return (uint32_t)pRegister[i] * 0x0400; };
		if (chrInversion) {
			pCHRBank[0] = r(2);
			pCHRBank[1] = r(3);
			pCHRBank[2] = r(4);
			...
			// nametables: chrInversion -> [R0,R0,R1,R1] (lub R2..R5 dla 4-screen)
			// Disch: gdy C set -> [R2,R3,R4,R5]
			ntA = (pRegister[2] >> 4) & 1; // bit 5 oryginalnej wartosci -> tu bit4 (po >>1?). Doc: "bit 5"
			ntB = (pRegister[3] >> 4) & 1;
			ntC = (pRegister[4] >> 4) & 1;
			ntD = (pRegister[5] >> 4) & 1;
		} else {
