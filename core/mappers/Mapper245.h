...
public:
	...
	void reset() override {
		...
		irqCounter = 0; irqReload = 0; lastA12 = false;
	...
	bool cpuMapRead(uint16_t addr, uint32_t& mapped, uint8_t&) override {
		...
		uint8_t bank;
		if (addr < 0xA000)      bank = prgMode ? prev : r6;
		else if (addr < 0xC000) bank = r7;
		else if (addr < 0xE000) bank = prgMode ? r6   : prev;
		else                    bank = last;
	...
	void cpuMapWrite(uint16_t addr, uint32_t&, uint8_t data) override {
		...
		if (addr < 0xA000) {
			...
			if ((addr & 1) == 0) irqReload = data;
			else { irqReloadFlag = true; irqCounter = 0; }
		} else {
			if ((addr & 1) == 0) { irqEnable = false; irqActive = false; }
			else                   irqEnable = true;
		}
	}
	...
	bool ppuMapRead(uint16_t addr, uint32_t& mapped) override {
		...
		if (chrBanks == 0) {
			...
			if (chrInversion) mapped = (uint32_t)((addr ^ 0x1000) & 0x1FFF);
			else              mapped = addr;
			return true;
		}
		...
		if (chrInversion) {
			...
			if (slot >= 4) { r = (uint8_t)((r & 0xFE) + (slot & 1)); }
