...
public:
	SDLWindow(ISDLWindowAPI* hardwareAPI) : platformAPI(hardwareAPI) {
		...
		texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_BGRA32,
			SDL_TEXTUREACCESS_STREAMING, NES::SCREEN_WIDTH, NES::SCREEN_HEIGHT);
	...
	~SDLWindow() override {
		// uiLayer zostanie zniszczone automatycznie tutaj (przed SDL), dzięki unique_ptr
	...
	bool pollEvent(AppEvent& out) override {
		...
		while (SDL_PollEvent(&ev)) {
			uiLayer->processEvent(&ev);
			if (toAppEvent(ev, out)) return true;
		}
		return false;
	}
	...
	// --- Renderowanie ---
	void presentNESFrame(const uint32_t* framebuffer, IFileSession& session) override {
		SDL_UpdateTexture(texture, nullptr, framebuffer, NES::SCREEN_WIDTH * sizeof(uint32_t));
...
private:
	static AppKey scancodeToAppKey(SDL_Scancode sc) {
		switch (sc) {
		case SDL_SCANCODE_P:     return AppKey::Pause;
		case SDL_SCANCODE_F11:   return AppKey::FullScreen;
		case SDL_SCANCODE_TAB:   return AppKey::SpeedUp;
		case SDL_SCANCODE_SPACE: return AppKey::NsfTogglePause;
		case SDL_SCANCODE_RIGHT: return AppKey::NsfNextSong;
		case SDL_SCANCODE_LEFT:  return AppKey::NsfPrevSong;
		default:                 return AppKey::Unknown;
		}
	}
	...
	bool toAppEvent(const SDL_Event& ev, AppEvent& out) {
		...
		case SDL_EVENT_KEY_DOWN:
			...
			if (ev.key.scancode == SDL_SCANCODE_TAB) {
				...
				out = { shift ? AppEventType::KeyDown : AppEventType::KeyDown };
				out.key = shift ? AppKey::SpeedDown : AppKey::SpeedUp;
				return true;
			}
			out = { AppEventType::KeyDown, scancodeToAppKey(ev.key.scancode) };
			return out.key != AppKey::Unknown;
		case SDL_EVENT_KEY_UP:
			if (ev.key.scancode == SDL_SCANCODE_TAB) {
				out = { AppEventType::KeyUp, AppKey::SpeedUp };
				return true;
			}
			{
				AppKey k = scancodeToAppKey(ev.key.scancode);
				if (k == AppKey::Unknown) return false;
