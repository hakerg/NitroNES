...
public:
    ...
    const char *tr(const char *id) const override {
        static const std::unordered_map<std::string, const char *> dict = {
            ...
            {"settings.sync_mode", "Emulation speed"},
            {"settings.sync.none", "Standard"},
            {"settings.sync.none.tooltip", "Runs the emulation at the console's original native speed.\nHIGHLY RECOMMENDED if you have a VRR (G-Sync / FreeSync) monitor for the smoothest experience and lowest input lag."},
            {"settings.sync.refresh_rate", "Monitor frequency"},
            {"settings.sync.refresh_rate.tooltip", "Adjusts emulation speed to match the monitor's refresh rate.\nRequires a monitor with a refresh rate close to 60Hz (NTSC) / 50Hz (PAL) or their multiples."},
