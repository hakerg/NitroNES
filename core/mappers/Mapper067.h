...
//   $C800: IRQ Load (write-twice: high first, then low). Bezposrednio ustawia licznik.
//   $D800: [...E ....] IRQ Enable (1=enable); jakikolwiek zapis kasuje IRQ i toggle.
//   $E800: [.... ..MM] mirroring (00=V,01=H,10=1ScA,11=1ScB)
...
public:
	...
	void reset() override {
		...
		irqCounter = 0; irqEnable = false; irqActive = false; irqHighNext = true;
	}
	...
	void cpuMapWrite(uint16_t addr, uint32_t&, uint8_t data) override {
		...
		switch (r) {
			...
			case 0xC800:
				if (irqHighNext) irqCounter = (uint16_t)((irqCounter & 0x00FF) | ((uint16_t)data << 8));
				else             irqCounter = (uint16_t)((irqCounter & 0xFF00) |  (uint16_t)data);
