...
public:
	...
	bool cpuMapRead(uint16_t a, uint32_t& mapped, uint8_t&) override {
		...
		uint8_t b;
		if      (a < 0xA000) b = R[6] & 0x3F;
		else if (a < 0xC000) b = R[7] & 0x3F;
	...
	bool ppuMapRead(uint16_t a, uint32_t& mapped) override {
		...
		uint8_t b;
		if      (a < 0x0800) b = R[2] & 0x3F;
		else if (a < 0x1000) b = R[3] & 0x3F;
		else if (a < 0x1800) b = R[4] & 0x3F;
		else                 b = R[5] & 0x3F;
