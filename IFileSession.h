...
public:
	...
	explicit IFileSession(const std::string& path, AppAudioStream& audio, IWindow& window, AppSettings& settings)
		: path(path)
	...
	double calcSpeedMultiplier(double baseSpeed) const {
		if (!core().hasPPU()) return 0.0;
		double monitorHz = window.getRefreshHz();
		if (monitorHz <= 0.0) return 0.0;

		double nesHz = core().pal ? 50.0 : NES::REFRESH_RATE_NTSC_ON; // TODO: proper pal support
		double ratio = monitorHz / (nesHz * baseSpeed);

		int n = canUseScanlineSync(baseSpeed) ? 1 : static_cast<int>(std::round(ratio));
		if (n < 1) return 0.0;
	...
	double adjustedSpeed(double baseSpeed) const {
		if (settings.matchRefreshRate) return baseSpeed;
