...
public:
    ...
    const char *tr(const char *id) const override {
        static const std::unordered_map<std::string, const char *> dict = {
            ...
            {"settings.sync_mode", "Prędkość emulacji"},
            {"settings.sync.none", "Standardowa"},
            {"settings.sync.none.tooltip", "Uruchamia emulację z oryginalną, natywną prędkością konsoli.\nZDECYDOWANIE ZALECANE, jeśli posiadasz monitor VRR (G-Sync / FreeSync), aby uzyskać idealną płynność i najniższe opóźnienie."},
            {"settings.sync.refresh_rate", "Częstotliwość monitora"},
            {"settings.sync.refresh_rate.tooltip", "Dostosowuje prędkość emulacji do odświeżania monitora.\nWymaga monitora o odświeżaniu bliskim 60Hz (NTSC) / 50Hz (PAL) lub ich wielokrotności."},
