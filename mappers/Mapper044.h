...
// Aby unikac kolizji z normalnym MMC3 PRG-RAM protect, czytamy bity bezposrednio.
// ----------------------------------------------------------------------------
...
public:
	...
	bool cpuMapRead(uint16_t addr, uint32_t& mapped, uint8_t& data) override {
		if (!Mapper004::cpuMapRead(addr, mapped, data)) return false;
		// Zaaplikuj OR bloku do offsetu PRG (8K paginacja MMC3).
		uint32_t prgAnd = (block == 6) ? 0x1FFFF : 0x0FFFF; // 128K vs 256K
		uint32_t prgOr;
		if (block <= 5) prgOr = (uint32_t)block * 128 * 1024;
		else            prgOr = 6u * 128u * 1024u;
	...
	bool ppuMapRead(uint16_t addr, uint32_t& mapped) override {
		...
		uint32_t chrOr;
		if (block <= 5) chrOr = (uint32_t)block * 128 * 1024;
		else            chrOr = 6u * 128u * 1024u;
