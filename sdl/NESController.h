...
public:
    ...
    SDL_JoystickID gamepadID() const {
        return gamepad_ ? SDL_GetGamepadID(gamepad_) : 0;
    }
    ...
    uint8_t readState() const {
        ...
        auto isDown = [&](SDL_Scancode sc) -> bool {
            return sc != SDL_SCANCODE_UNKNOWN && sc < (SDL_Scancode)numKeys &&
                   ks[sc];
        };
        ...
        bool turboAdown =
            isDown(keys.key_TurboA) || padBtn(SDL_GAMEPAD_BUTTON_WEST);
        bool turboBdown =
            isDown(keys.key_TurboB) || padBtn(SDL_GAMEPAD_BUTTON_NORTH);
        ...
        if (padBtn(SDL_GAMEPAD_BUTTON_SOUTH))
            state |= 0x80;
        if (padBtn(SDL_GAMEPAD_BUTTON_EAST))
            state |= 0x40;
        if (padBtn(SDL_GAMEPAD_BUTTON_BACK))
            state |= 0x20;
        if (padBtn(SDL_GAMEPAD_BUTTON_START))
            state |= 0x10;
        if (padBtn(SDL_GAMEPAD_BUTTON_DPAD_UP))
            state |= 0x08;
        if (padBtn(SDL_GAMEPAD_BUTTON_DPAD_DOWN))
            state |= 0x04;
        if (padBtn(SDL_GAMEPAD_BUTTON_DPAD_LEFT))
            state |= 0x02;
        if (padBtn(SDL_GAMEPAD_BUTTON_DPAD_RIGHT))
            state |= 0x01;
    ...
    void tickFrame() {
        ...
        bool turboAdown = isDown(keys.key_TurboA) ||
                          (gamepad_ && SDL_GetGamepadButton(
                                           gamepad_, SDL_GAMEPAD_BUTTON_WEST));
        bool turboBdown = isDown(keys.key_TurboB) ||
                          (gamepad_ && SDL_GetGamepadButton(
