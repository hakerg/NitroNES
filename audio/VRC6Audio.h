...
public:
	...
	void writeReg(uint16_t reg, uint8_t data) {
		switch (reg & 0xF003) {
			case 0x9000: pulse_[0].writeCtrl(data); break;
			case 0x9001: pulse_[0].writeFreqLo(data); break;
			case 0x9002: pulse_[0].writeFreqHi(data); break;
			case 0x9003: writeSoundCtrl(data); break;

			case 0xA000: pulse_[1].writeCtrl(data); break;
			case 0xA001: pulse_[1].writeFreqLo(data); break;
			case 0xA002: pulse_[1].writeFreqHi(data); break;

			case 0xB000: saw_.writeAccum(data); break;
			case 0xB001: saw_.writeFreqLo(data); break;
			case 0xB002: saw_.writeFreqHi(data); break;
	...
	void clock() {
		...
		if (freqShift_ == 0) {
			...
			if ((freqShiftCounter_++ & mask) == 0) tickChannels();
		}
	}

	// Aktualne wyjscie audio jako float ~[0, 1).
	float output() const {
		if (soundHalt_) return 0.0f;
		uint32_t sum = (uint32_t)pulse_[0].output()
					 + (uint32_t)pulse_[1].output()
					 + (uint32_t)saw_.output();
...
private:
	...
	struct Pulse {
		...
		void writeFreqHi(uint8_t d) {
			...
			bool en = (d & 0x80) != 0;
			if (!enabled && en) { step = 0; timer = freq; }
			enabled = en;
		}
		void clock() {
			if (!enabled) return;
			if (timer == 0) { timer = freq; step = (uint8_t)((step + 1) & 0x0F); }
			else            { timer--; }
		}
		uint8_t output() const {
			if (!enabled) return 0;
			if (gate) return volume; // tryb digitized
	...
	struct Saw {
		...
		void writeFreqHi(uint8_t d) {
			freq = (uint16_t)((freq & 0x00FF) | ((d & 0x0F) << 8));
			bool en = (d & 0x80) != 0;
			if (!enabled && en) { accum = 0; step = 0; timer = freq; }
			enabled = en;
		}
		void clock() {
			if (!enabled) return;
			if (timer == 0) {
				...
				step++;
				if ((step & 0x01) == 0) accum = (uint8_t)(accum + accumRate);
				if (step >= 14) { step = 0; accum = 0; }
			} else timer--;
		}
		uint8_t output() const {
			if (!enabled) return 0;
