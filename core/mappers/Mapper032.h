...
// ----------------------------------------------------------------------------
// PRG: 8 KB switchable @ $8000 (R0), 8 KB switchable @ $A000 (R1), {-2,-1} fixed.
// W trybie 1: R0 mapuje $C000, {-2} fixed @ $8000.
// CHR: 8x 1 KB switchable.
// $9000: bit0 = PRG mode, bit1 = mirroring (0=V, 1=H).
// ----------------------------------------------------------------------------
...
public:
	...
	bool cpuMapRead(uint16_t addr, uint32_t& mapped, uint8_t&) override {
		...
		uint8_t bank;
		if (addr < 0xA000)      bank = prgMode ? prev   : prg0;
		else if (addr < 0xC000) bank = prg1;
		else if (addr < 0xE000) bank = prgMode ? prg0   : prev;
		else                    bank = lastPrg;
	...
	void cpuMapWrite(uint16_t addr, uint32_t&, uint8_t data) override {
		...
		switch (addr & 0xF000) {
		case 0x8000: prg0 = data & 0x1F; break;
		case 0x9000:
			prgMode    = (data & 0x02) != 0;
			mirrorMode = (data & 0x01) ? Mirroring::HORIZONTAL : Mirroring::VERTICAL;
			break;
		case 0xA000: prg1 = data & 0x1F; break;
		case 0xB000: chr[addr & 0x07] = data; break;
		default: break;
		}
	}
