...
public:
	...
	void reset() override { prgBankSelect = 0; mirrorMode = Mirroring::ONESCREEN_LO; }
	...
	void cpuMapWrite(uint16_t addr, uint32_t&, uint8_t data) override {
		if (addr >= 0x8000) {
			...
			mirrorMode = (data & 0x10) ? Mirroring::ONESCREEN_HI : Mirroring::ONESCREEN_LO;
	...
	// ANROM ma, ale gry ANROM zapisuja "bezpieczne" wartosci, wiec false jest OK
	// dla calej rodziny. Wlaczone konflikty rozwalaja Battletoads (zly bank PRG
	// po wyborze levelu -> czarny ekran, glitch statku w intro).
