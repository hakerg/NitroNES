...
public:
	...
	bool cpuMapRead(uint16_t addr, uint32_t& mapped, uint8_t&) override {
		...
		uint8_t bank;
		if      (addr < 0xA000) bank = prg0;
		else if (addr < 0xC000) bank = prg1;
		else if (addr < 0xE000) bank = (uint8_t)(total8k - 2);
	...
	void cpuMapWrite(uint16_t addr, uint32_t&, uint8_t data) override {
		...
		switch (reg) {
			case 0x8000: prg0 = data & 0x3F; break;
			case 0x8001: prg1 = data & 0x3F; break;
			case 0x8002: chr0 = data; break;
			case 0x8003: chr1 = data; break;
			case 0xA000: chr2 = data; break;
			case 0xA001: chr3 = data; break;
			case 0xA002: chr4 = data; break;
			case 0xA003: chr5 = data; break;
			case 0xC000: irqReload = (uint8_t)(data ^ 0xFF); break;
			case 0xC001: irqReloadFlag = true; irqCounter = 0; break;
			case 0xC002: irqEnable = true; break;
			case 0xC003: irqEnable = false; irqActive = false; break;
			case 0xE000:
	...
	bool ppuMapRead(uint16_t addr, uint32_t& mapped) override {
		...
		if (addr < 0x0800) {
			...
			bank1k = base + ((addr >> 10) & 0x01);
		} else if (addr < 0x1400) bank1k = chr2;
		else if   (addr < 0x1800) bank1k = chr3;
		else if   (addr < 0x1C00) bank1k = chr4;
		else                      bank1k = chr5;
...
private:
	void tickIrq() {
		if (irqCounter == 0 || irqReloadFlag) { irqCounter = irqReload; irqReloadFlag = false; }
		else irqCounter--;
	...
	bool irqActive = false, irqEnable = false, irqReloadFlag = false, lastA12 = false;
