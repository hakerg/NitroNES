...
// ----------------------------------------------------------------------------
// PRG: 8KB switchable, trzy koñcowe banki na sta³e (-3, -2, -1).
// CHR: dwie 4KB strefy, ka¿da z dwóch banków wybieranych automatycznie przez
...
public:
	...
	void cpuMapWrite(uint16_t addr, uint32_t&, uint8_t data) override {
		...
		switch (addr & 0xF000) {
			...
			case 0xB000: chrBank0A = data & 0x1F; break;
			case 0xC000: chrBank0B = data & 0x1F; break;
			case 0xD000: chrBank1A = data & 0x1F; break;
			case 0xE000: chrBank1B = data & 0x1F; break;
			case 0xF000:
	...
	bool ppuMapRead(uint16_t addr, uint32_t& mapped) override {
		...
		// Wybór banku wed³ug aktualnego stanu latcha
		if (addr < 0x1000) {
		...
		if (addr == 0x0FD8)                     latch0 = 0xFD;
		else if (addr == 0x0FE8)                latch0 = 0xFE;
