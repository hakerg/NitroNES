...
public:
	...
	void cpuMapWrite(uint16_t addr, uint32_t& mapped, uint8_t data) override {
		if (addr < 0x8000) { Mapper004::cpuMapWrite(addr, mapped, data); return; }
		uint16_t reg = addr & 0xE001;
		switch (reg) {
			case 0x8001: Mapper004::cpuMapWrite(0xA000, mapped, data); return;     // mirroring
			case 0xA000: {
			...
			case 0xC000: Mapper004::cpuMapWrite(0x8001, mapped, data); return;     // bank data
			case 0xC001:
				Mapper004::cpuMapWrite(0xC000, mapped, data);
				Mapper004::cpuMapWrite(0xC001, mapped, data);
				return;
			case 0xE000: Mapper004::cpuMapWrite(0xE000, mapped, data); return;
			case 0xE001: Mapper004::cpuMapWrite(0xE001, mapped, data); return;
			default: return;
		}
	}
};
