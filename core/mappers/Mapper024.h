...
public:
	...
	void reset() override {
		prg16 = 0; prg8 = 0;
		for (auto& r : chrReg) r = 0;
		bMode = 0; ppuExt = false; ppuA10rule = true; prgRamEnable = false;
		irqLatch = 0; irqMode = false; irqEnable = false; irqEnableOnAck = false;
		irqCounter = 0; irqPrescaler = 341; irqActive = false;
	...
	bool ppuMapRead(uint16_t addr, uint32_t& mapped) override {
		...
		switch (bMode & 0x03) {
			default:
			case 0: bank = chrReg[slot]; break;
			case 1: {
				static const uint8_t map[8] = { 0,0,1,1,2,2,3,3 };
				uint8_t r = chrReg[map[slot]];
				if (ppuA10rule) bank = (uint8_t)((r & 0xFE) | ((addr >> 10) & 0x01));
				else            bank = r;
				break;
			}
			case 2: case 3: {
				if (slot < 4) bank = chrReg[slot];
				else {
...
private:
	...
	void writeReg(uint16_t reg, uint8_t data) {
		switch (reg & 0xF003) {
			...
			case 0xB000: case 0xB001: case 0xB002:
				...
				break;
			case 0xA003: /* niewykorzystane w VRC6 audio */ break;
			...
			case 0xB003:
				...
				switch (data & 0x0C) {
					case 0x00: mirrorMode = Mirroring::VERTICAL; break;
					case 0x04: mirrorMode = Mirroring::HORIZONTAL; break;
					case 0x08: mirrorMode = Mirroring::ONESCREEN_LO; break;
					case 0x0C: mirrorMode = Mirroring::ONESCREEN_HI; break;
				}
				break;

			case 0xC000: case 0xC001: case 0xC002: case 0xC003:
				prg8 = data & 0x1F; break;

			case 0xD000: chrReg[0] = data; break;
			case 0xD001: chrReg[1] = data; break;
			case 0xD002: chrReg[2] = data; break;
			case 0xD003: chrReg[3] = data; break;
			case 0xE000: chrReg[4] = data; break;
			case 0xE001: chrReg[5] = data; break;
			case 0xE002: chrReg[6] = data; break;
			case 0xE003: chrReg[7] = data; break;

			case 0xF000: irqLatch = data; break;
			case 0xF001:
				irqMode        = (data & 0x04) != 0;
				irqEnable      = (data & 0x02) != 0;
				irqEnableOnAck = (data & 0x01) != 0;
				if (irqEnable) { irqCounter = irqLatch; irqPrescaler = 341; }
				irqActive = false;
				break;
			case 0xF002:
				irqActive = false;
	...
	void tickIrqCounter() {
		if (irqCounter == 0xFF) { irqCounter = irqLatch; irqActive = true; }
		else                    { irqCounter++; }
	}
	...
	bool irqMode = false, irqEnable = false, irqEnableOnAck = false, irqActive = false;
