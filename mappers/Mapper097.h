...
public:
	using Mapper::Mapper;
	void reset() override { prg = 0; mirrorMode = Mirroring::ONESCREEN_LO; }
	bool cpuMapRead(uint16_t a, uint32_t& mapped, uint8_t&) override {
		if (a < 0x8000) return false;
		if (a < 0xC000) mapped = (uint32_t)(prgBanks - 1) * 0x4000 + (a & 0x3FFF);
		else            mapped = (uint32_t)prg * 0x4000 + (a & 0x3FFF);
		return true;
	}
