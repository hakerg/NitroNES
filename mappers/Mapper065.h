...
public:
	...
	void reset() override {
		...
		irqEnable = false; irqActive = false;
		irqReload = 0; irqCounter = 0;
	}
	...
	void cpuMapWrite(uint16_t a, uint32_t&, uint8_t data) override {
		...
		switch (a) {
			...
			case 0xC000: prg[2] = data; break;
			case 0x9001: mirrorMode = (data & 0x80) ? Mirroring::HORIZONTAL : Mirroring::VERTICAL; break;
			case 0x9003: irqEnable = (data & 0x80) != 0; irqActive = false; break;
			case 0x9004: irqCounter = irqReload; irqActive = false; break;
			case 0x9005: irqReload = (uint16_t)((irqReload & 0x00FF) | (data << 8)); break;
			case 0x9006: irqReload = (uint16_t)((irqReload & 0xFF00) | data); break;
			default:
	...
	void clock() override {
		if (!irqEnable || irqCounter == 0) return;
		if (--irqCounter == 0) irqActive = true;
	}
