...
public:
	...
	void reset() override {
		loadReg = 0x00; loadCount = 0;
		controlReg = 0x1C;          // 16k PRG, $8000 swap, 1-screen lo (bity 2,3 ustawione)
	...
	bool cpuMapRead(uint16_t addr, uint32_t& mapped, uint8_t&) override {
		...
		if (addr < 0x8000) {
			// 8KB PRG-RAM (obsluzona w Cartridge), tu sygnalizujemy ze to nie ROM
			return false;
		}
	...
	void cpuMapWrite(uint16_t addr, uint32_t& mapped, uint8_t data) override {
		...
		if (data & 0x80) {
			...
			// gra (np. testy Blargga z MMC1) zawiesza sie w petli inicjalizacji.
			loadReg = 0x00;
			loadCount = 0;
			controlReg |= 0x0C;
			(void)mapped;
		...
		if (loadCount == 5) {
			uint8_t targetReg = (addr >> 13) & 0x03; // 0:8000-9FFF 1:A000-BFFF 2:C000-DFFF 3:E000-FFFF
			switch (targetReg) {
				case 0:
					...
					break;
				case 1: chrBank0   = loadReg & 0x1F; break;
				case 2: chrBank1   = loadReg & 0x1F; break;
				case 3: prgBankReg = loadReg & 0x1F; break;
			}
			loadReg = 0; loadCount = 0;
		}
		(void)mapped;
	}
	...
	bool ppuMapRead(uint16_t addr, uint32_t& mapped) override {
		...
		if (chrBanks == 0) { mapped = addr; return true; }
