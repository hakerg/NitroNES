...
// Mapper 089 - Sunsoft. Bus conflicts.
class Mapper089 : public Mapper {
public:
	using Mapper::Mapper;
	void reset() override { prg = 0; chr = 0; mirrorMode = Mirroring::ONESCREEN_LO; }
	...
	void cpuMapWrite(uint16_t a, uint32_t&, uint8_t data) override {
		if (a < 0x8000) return;
		chr = (uint8_t)((data & 0x07) | ((data & 0x80) >> 4));
		prg = (data >> 4) & 0x07;
		mirrorMode = (data & 0x08) ? Mirroring::ONESCREEN_HI : Mirroring::ONESCREEN_LO;
