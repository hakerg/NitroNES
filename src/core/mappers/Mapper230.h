...
// Reset toggluje tryb. Bez wsparcia softreset cycle, startujemy w trybie
// multicart. Contra Mode  $8000-FFFF: [.... .PPP] - 16K page z chip0 + fixed
// page 7 @ $C000. Multi Mode   $8000-FFFF: [.MOP PPPP]
...
public:
    ...
    bool cpuMapRead(uint16_t addr, uint32_t &mapped, uint8_t &) override {
        ...
        uint32_t off;
        if (mode16)
            off = chipBase + (uint32_t)p * 0x4000 + (addr & 0x3FFF);
        else
            off = chipBase + (uint32_t)(p >> 1) * 0x8000 + (addr & 0x7FFF);
...
public:
    std::unique_ptr<Mapper> clone() const override { return std::make_unique<Mapper230>(*this); }
