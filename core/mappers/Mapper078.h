...
// Mirroring jest jednoekranowy lub H/V w zale¿noœci od gry – wybieramy
// uproszczone H/V (dzia³a dla Holy Divera; dla Cosmo Carriera mo¿na rêcznie
// podmieniæ mapper na wariant 1ScA/1ScB).
class Mapper078 : public Mapper {
public:
	using Mapper::Mapper;
	void reset() override { prg = 0; chr = 0; mirrorMode = Mirroring::HORIZONTAL; }
	...
	void cpuMapWrite(uint16_t a, uint32_t&, uint8_t data) override {
		if (a < 0x8000) return;
		prg = data & 0x07;
		mirrorMode = (data & 0x08) ? Mirroring::VERTICAL : Mirroring::HORIZONTAL;
		chr = (data >> 4) & 0x0F;
