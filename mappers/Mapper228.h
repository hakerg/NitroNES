...
//
// Uproszczenie: traktujemy PRG jako liniowe (bez special-casingu chip 2 open bus).
// Reordered chip map (0,1,2,3) -> w pliku iNES kolejne 512 KB regionow.
// ----------------------------------------------------------------------------
...
public:
	...
	void reset() override {
		prgChip = 0; prgPage = 0; mode16 = false;
		chrLo = 0; chrHi = 0;
	...
	bool cpuMapRead(uint16_t addr, uint32_t& mapped, uint8_t&) override {
		...
		if (chip == 2) chip = 1; // chip 2 nie istnieje - mapujemy na chip 1 zamiast open bus
		uint32_t chipBase = (uint32_t)chip * (512 * 1024);
		uint32_t off;
		if (mode16) {
			off = chipBase + (uint32_t)prgPage * 0x4000 + (addr & 0x3FFF);
		} else {
			off = chipBase + (uint32_t)(prgPage >> 1) * 0x8000 + (addr & 0x7FFF);
		}
