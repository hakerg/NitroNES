...
public:
	...
	bool cpuMapRead(uint16_t addr, uint32_t& mapped, uint8_t& data) override {
		...
		if (inRom) outMapped = (uint32_t)bank * 0x8000 + (addr & 0x7FFF);

		bankswitchOnAccess(addr);

		if (!inRom) return false;
		mapped = outMapped;
		(void)data;
		return true;
	}
...
private:
	...
	uint8_t currentPrg() const {
		...
		bool O = (reg0 & 0x40) != 0;
		uint8_t bank;
		if (!O) bank = (uint8_t)((B << 1) | b);
		else    bank = (uint8_t)((B << 1) | (reg2 & 0x01));
		// Q (bit 5) selects ROMs 3+4; ignorujemy (kartridze dystrybuowano tylko z 1+2).
		return bank;
	}
	uint8_t currentChr() const {
		...
		bool O = (reg0 & 0x40) != 0;
		if (!O) return (uint8_t)((B << 4) | (b << 3) | (CC << 1) | (CC >> 1));
