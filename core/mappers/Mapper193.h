...
public:
	...
	bool cpuMapRead(uint16_t addr, uint32_t& mapped, uint8_t&) override {
		...
		uint8_t bank;
		if      (addr < 0xA000) bank = prgReg;
		else if (addr < 0xC000) bank = (uint8_t)(total8k - 3);
	...
	void cpuMapWrite(uint16_t addr, uint32_t&, uint8_t data) override {
		if (addr >= 0x6000 && addr <= 0x7FFF) {
			switch (addr & 0x6003) {
				case 0x6000: chr0 = data; break;
				case 0x6001: chr1 = data; break;
				case 0x6002: chr2 = data; break;
				case 0x6003: prgReg = data; break;
			}
		}
	}
	...
	bool ppuMapRead(uint16_t addr, uint32_t& mapped) override {
		...
		uint32_t bank1k;
		if (addr < 0x1000) {
			// 4 KB bank: chr0 indexes 4 KB units (bits 0..1 ignored per typical doc).
