...
public:
    ...
    bool cpuMapRead(uint16_t a, uint32_t &mapped, uint8_t &) override {
        ...
        if (a < 0xA000)
            mapped = (uint32_t)prg0 * 0x2000 + (a & 0x1FFF);
        else if (a < 0xC000)
            mapped = (uint32_t)prg1 * 0x2000 + (a & 0x1FFF);
        else if (a < 0xE000)
            mapped = (total8k - 2) * 0x2000 + (a & 0x1FFF);
        else
            mapped = (total8k - 1) * 0x2000 + (a & 0x1FFF);
        return true;
    }
    ...
    bool ppuMapRead(uint16_t a, uint32_t &mapped) override {
        if (a > 0x1FFF)
            return false;
        if (a < 0x0800)
            mapped = (uint32_t)chr2k[0] * 0x0800 + (a & 0x07FF);
        else if (a < 0x1000)
            mapped = (uint32_t)chr2k[1] * 0x0800 + (a & 0x07FF);
        else {
