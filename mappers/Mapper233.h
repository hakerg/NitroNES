...
public:
	...
	bool cpuMapRead(uint16_t addr, uint32_t& mapped, uint8_t&) override {
		...
		if (mode32) {
			...
			uint32_t off = (uint32_t)mapper_helpers::maskBank(p, prgBanks) * 0x4000;
	...
	Mirroring mirror() const override {
		switch ((reg >> 6) & 0x03) {
			case 0:  return Mirroring::ONESCREEN_LO; // dirty approximation
			case 1:  return Mirroring::VERTICAL;
			case 2:  return Mirroring::HORIZONTAL;
			default: return Mirroring::ONESCREEN_HI;
		}
	}
