...
// Implementujemy podstawow� logik� bank�w + IRQ (zar�wno tryb scanline jak i CPU).
class Mapper064 : public Mapper {
public:
	using Mapper::Mapper;
	void reset() override {
		bankSel = 0; chrMode = false; prgMode = false; k1k = false;
		for (auto& r : R) r = 0;
		mirrorMode = Mirroring::HORIZONTAL;
		irqReload = 0; irqCounter = 0; irqEnable = false; irqActive = false;
		irqReloadFlag = false; irqCpuMode = false;
		prescaler = 0; lastA12 = false;
	}
	...
	bool cpuMapRead(uint16_t a, uint32_t& mapped, uint8_t&) override {
		...
		uint8_t bank;
		if (!prgMode) {
			if      (a < 0xA000) bank = R[6];
			else if (a < 0xC000) bank = R[7];
			else if (a < 0xE000) bank = R[15];
			else                 bank = (uint8_t)(total8k - 1);
		} else {
			if      (a < 0xA000) bank = R[15];
			else if (a < 0xC000) bank = R[6];
			else if (a < 0xE000) bank = R[7];
			else                 bank = (uint8_t)(total8k - 1);
		}
	...
	void cpuMapWrite(uint16_t a, uint32_t&, uint8_t data) override {
		...
		switch (reg) {
			...
			case 0xC001:
				...
				prescaler = 0;
				break;
			case 0xE000: irqEnable = false; irqActive = false; break;
	...
	bool ppuMapRead(uint16_t a, uint32_t& mapped) override {
		...
		if (!chrMode) {
			if (a < 0x0800)      bank = k1k ? ((a < 0x0400) ? R[0] : R[8]) : (uint8_t)((R[0] & 0xFE) | ((a >> 10) & 1));
			else if (a < 0x1000) bank = k1k ? ((a < 0x0C00) ? R[1] : R[9]) : (uint8_t)((R[1] & 0xFE) | ((a >> 10) & 1));
			else if (a < 0x1400) bank = R[2];
			else if (a < 0x1800) bank = R[3];
			else if (a < 0x1C00) bank = R[4];
			else                 bank = R[5];
		} else {
			if      (a < 0x0400) bank = R[2];
			else if (a < 0x0800) bank = R[3];
			else if (a < 0x0C00) bank = R[4];
			else if (a < 0x1000) bank = R[5];
			else if (a < 0x1800) bank = k1k ? ((a < 0x1400) ? R[0] : R[8]) : (uint8_t)((R[0] & 0xFE) | ((a >> 10) & 1));
			else                 bank = k1k ? ((a < 0x1C00) ? R[1] : R[9]) : (uint8_t)((R[1] & 0xFE) | ((a >> 10) & 1));
		}
	...
	void clock() override {
		if (!irqCpuMode) return;
		if (++prescaler >= 4) { prescaler = 0; tickIrq(); }
	}
...
private:
	...
	bool irqEnable = false, irqActive = false, irqReloadFlag = false, irqCpuMode = false;
	uint8_t prescaler = 0;
	bool lastA12 = false;
};
