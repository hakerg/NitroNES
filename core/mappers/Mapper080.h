...
public:
	...
	void reset() override {
		chr.fill(0); prg.fill(0); ram.fill(0);
	...
	bool cpuMapRead(uint16_t addr, uint32_t& mapped, uint8_t& data) override {
		if (addr >= 0x7F00 && addr <= 0x7FFF) {
			if (ramEnabled) data = ram[addr & 0x7F];
			else            data = 0xFF;
		...
		else if (addr < 0xE000) bank = prg[2];
		else                    bank = last;
	...
	void cpuMapWrite(uint16_t addr, uint32_t&, uint8_t data) override {
		if (addr >= 0x7F00 && addr <= 0x7FFF) {
			if (ramEnabled) ram[addr & 0x7F] = data;
			return;
		}
		if (addr < 0x7EF0 || addr > 0x7EFF) return;
		uint16_t r = addr & 0x000F;
		switch (r) {
			case 0x0: case 0x1: case 0x2: case 0x3: case 0x4: case 0x5:
				chr[r] = data; break;
			case 0x6: case 0x7:
				mirrorMode = (data & 0x01) ? Mirroring::VERTICAL : Mirroring::HORIZONTAL;
				break;
			case 0x8: case 0x9:
				ramEnabled = (data == 0xA3);
				break;
			case 0xA: case 0xB: prg[0] = data; break;
			case 0xC: case 0xD: prg[1] = data; break;
			case 0xE: case 0xF: prg[2] = data; break;
		}
	}
...
// Mapper 207 (Fudou Myouou Den) - bardzo zblizony, brak 1-screen variant. Alias roboczy:
