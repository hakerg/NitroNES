...
// Mapper 073 - VRC3. 16-bit licznik IRQ napêdzany cyklem CPU.
...
public:
	...
	void reset() override {
		prg = 0;
		reload = 0; counter = 0;
		irqMode = false; irqEnable = false; irqEnableOnAck = false;
		irqActive = false;
	}
	...
	void cpuMapWrite(uint16_t a, uint32_t&, uint8_t data) override {
		if (a < 0x8000) return;
		switch (a & 0xF000) {
			case 0x8000: reload = (uint16_t)((reload & 0xFFF0) | (data & 0x0F)); break;
			case 0x9000: reload = (uint16_t)((reload & 0xFF0F) | ((data & 0x0F) << 4)); break;
			case 0xA000: reload = (uint16_t)((reload & 0xF0FF) | ((data & 0x0F) << 8)); break;
			case 0xB000: reload = (uint16_t)((reload & 0x0FFF) | ((data & 0x0F) << 12)); break;
			case 0xC000:
				irqMode        = (data & 0x04) != 0;
				irqEnable      = (data & 0x02) != 0;
				irqEnableOnAck = (data & 0x01) != 0;
				irqActive      = false;
				if (irqEnable) counter = reload;
				break;
			case 0xD000:
				irqActive = false;
				irqEnable = irqEnableOnAck;
				break;
			case 0xF000: prg = data & 0x0F; break;
		}
